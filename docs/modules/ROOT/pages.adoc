...
. Download the latest Windows Installer file from link:++https://fedoraproject.org++[fedoraproject.org]. The server automatically detects the running system and offers you the correct installation file for your Windows version.
...
+
If you get an error about a missing .dll file (the MSVC redistributable), the needed library is freely available here: link:++https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist++[Microsoft Visual C++ Redistributable latest supported downloads].


==== On macOS

. Download the latest Macintosh Disk Image (the package will have a .dmg extension) from link:++https://fedoraproject.org++[fedoraproject.org]. The server automatically detects the running system and offers a correct package for your macOS.
...
+
image::fedora_media_writer/FMW-select-image-source1.png[Image of Fedora Media Writer Main Screen]
+
...
+
image::fedora_media_writer/FMW-select-release-2.png[Image of Fedora Media Writer Distro Information Screen]
+
...
+
image::fedora_media_writer/FMW-download-options-3.png[Image of Fedora Media Writer Automatic Download]
...
+
image::fedora_media_writer/FMW-writing-5.png[Image of Fedora Media Writer write to device red button]
...
By calculating the image's `checksum` on your own computer and comparing it to the original `checksum`, you can verify the image has not been tampered with or corrupted. The original checksum values are provided at link:++https://fedoraproject.org/security/++[], and are [command]#gpg# signed to demonstrate their integrity.

[[sect-verifying-windows]]
==== Verifying checksums on Windows systems

. Download the Fedora image of your choice from link:++https://fedoraproject.org/++[] and the corresponding checksum file from link:++https://fedoraproject.org/security/++[]
...
function Get-ImageVerification(){
    ...
    $expected_checksum = ((Get-Content $ChecksumFilePath | Select-String -Pattern (Split-Path $ImageFilePath -leaf)) -split " ")[-1].ToLower()
    $download_checksum = (Get-FileHash -Path $ImageFilePath -Algorithm SHA256).Hash.ToLower()
...
+
----
Get-ImageVerification .\Fedora-Server-DVD-x86_64-21.iso .\Fedora-Server-21-x86_64-CHECKSUM
----

[[sect-verifying-nix]]
==== Verifying checksums on Linux and macOS systems

. Download the Fedora image of your choice from link:++https://fedoraproject.org/get-fedora++[] and the corresponding checksum file from link:++https://fedoraproject.org/security/++[]
